            ...
            <table className="w-full border-collapse text-left">
                <thead>
                    <tr className="bg-theme-card border-b border-theme-border">
                        {columns.map((header, index) => (
                            <th key={index} className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-theme-muted">
                                {header}
                            </th>
                        ))}
                    </tr>
                ...
                <tbody className="divide-y divide-theme-border bg-theme-main">
                    {data.map((row, rowIndex) => {
                        ...
                        return (
                            <tr key={row.id || rowIndex} className={`  ${striped && rowIndex % 2 === 0 ? 'bg-theme-card' : ''} ${hover ? 'hover:bg-theme-border/50' : ''} transition-colors`} >
                                {cells.map((cell, cellIndex) => {
                                    const header = columns[cellIndex];
                                    const headerLower = header.toLowerCase();
                                    ...
                                    const isImageColumn = headerLower.includes('logo') ||
                                        ...
                                        return (
                                            <td key={cellIndex} className={`px-6 py-4 text-sm text-theme-text ${isImageColumn ? 'whitespace-normal' : 'whitespace-nowrap'} `}>
                                                {shouldShowImage ? ( 
                                                    <Image src={cell} alt={header} className="h-12 w-12 object-contain rounded-lg shadow-sm bg-white/10"
                                                        onError={(e) => {
                                                            e.target.style.display = 'none';
                                                            e.target.nextSibling.style.display = 'block';
                                                        }}
